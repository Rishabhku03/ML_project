...
must_haves:
  truths:
    - "Synthetic generator produces ~10K rows of Zulip-style messages"
...
<objective>
Create the synthetic data generator that produces ~10K rows of Zulip-style messages via HuggingFace API.

Purpose: Expand the dataset from 1.58M to ~1.59M rows with synthetic data that rebalances minority classes (toxic/suicide). The synthetic data looks like realistic Zulip chat messages with prompt-guided labels.
...
- D-10: Target volume: ~10K synthetic rows (representative sample; full-scale deferred)
...
<task type="auto">
  ...
  <action>
    ...
    - Generates ~10K rows with 30% toxic, 30% suicide, 40% benign (D-10, D-11)
